...
- **SAS attestations + Examiner View:** every evaluated payment (blocks included) emits a Solana Attestation Service attestation (`sas-lib`, program `22zoJMtdu4tQc2PzL74ZUT7FrwgB1Udec8DdW4yw4BdG`) pinning `registry_version` and a `citation_hash` (SHA-256 over Borsh of the ordered citation set). The Examiner View (`app/examiner`) is a **Next.js app deployed to Vercel** — deliberately tiny: one dynamic route `/a/[address]` (server component fetches attestation + registry record from devnet RPC) plus an address-input home page. Server-rendered OG metadata so shared links preview as audit documents. Style it as a *document* (typographic, citation blocks, visible disclaimer), never a dashboard; no wallet connection, no interactivity. It is the judge-facing artifact and the PRD §9 shareable-link deliverable. Build/polish against fixture JSON so it's never blocked by live SAS integration.
